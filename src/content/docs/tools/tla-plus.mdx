...
- `db` is the `db` variable in `lazyConnect`. `connecting` is outside the code: the `connect()` calls that have not
  returned yet.
- Each action is one step of the code between two `await`s. `call c` is `getDb()` up to `await connect()`; the two
  `connect` actions are the two ways that call can end.
- The rules come before anything is checked. "At most one connect() at a time" is a "must never happen". "Every
  caller gets the connection in the end" is a "must always happen in the end", with what it takes for granted.
...
- `Call` calls the real `getDb()`. The `Connect` steps make a waiting `connect()` call resolve or reject. If none is
  waiting, the code and the spec disagree, and the test says so.
- After each step, the `connect()` calls in flight and each caller's status must match the spec, and there must
  never be two connects at once.
