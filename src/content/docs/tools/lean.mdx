...
:::note
`applyDiscount` is a pure function: the same inputs always give the same output, and it changes nothing outside
itself. No mutation of its arguments or of shared state, no I/O, no clock, no randomness. That is what makes it easy to
copy into Lean.
:::
