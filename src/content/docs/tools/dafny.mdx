...
- A check of `requires` at runtime. Dafny proves the function is right when the caller keeps the promise. A TS caller
  ...
  there once.
- JavaScript that looks written by hand. The output is readable but generated (`_0_lo`,
  `_dafny.EuclideanDivisionNumber`). You do not edit it; you edit the `.dfy` file.
- Proofs without effort. Loops need invariants, and writing them is the main skill to learn. The good part: a failed
  invariant points at a line, and the fix is usually a sentence you already had in your head.
