...
smaller description of the system in a `.qnt` file. That sounds like extra work, and it is, but it is worth it for three
reasons:
...
- Freedom. You choose what to leave out and how large the steps are. You can try a different design (a request
...
- A thinking tool. To write the model you have to name every state, every step and every "must never happen". Many
  ...
  cannot get out of date without CI noticing, once the replay in Step 5 is in place.

The cost is a second artifact to keep in sync with the code. The rest of this page shows how to do that with a replay
test in vitest.
...
- A model without effort. You choose where the steps split. This model splits `onInput` at its one `await`. If you
  merge steps that the real code keeps apart, the model hides the bug.
