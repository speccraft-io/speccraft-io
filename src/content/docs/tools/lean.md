...
Create a project next to your TS code:
...
```sh
lake exe discount > ../ts-shop/lean/cases.json
```
...
Commit `cases.json`. When someone changes the Lean file, they regenerate it. When someone changes the TS file, the
cross-check runs in CI like any other test.
