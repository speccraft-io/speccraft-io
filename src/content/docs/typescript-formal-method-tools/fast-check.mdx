...
- **A clean run is not a proof.** It tries 100 random runs by default, so a bug that needs one rare order can be
  missed. For every order, [pnueli](/typescript-formal-method-tools/pnueli), [TLA+](/formal-method-tools/tla-plus) or [Quint](/formal-method-tools/quint) check a model instead.
