...
One `.tla` file works with both, which is why this is one page and not three. [Quint](/formal-method-tools/quint) is a friendlier
...
TLA+ is not the tool for proving one function's math right for every input. See the [Dafny](/formal-method-tools/dafny) and
...
[Quint page](/formal-method-tools/quint) for a replay built on those).
...
- **Liveness, not only safety.** "Every caller gets the connection in the end" is checked, not only "nothing bad
  happens".
- **A short, readable failing trace.** You get the shortest order that breaks the spec, not a bug that shows up once in
  a while at startup.
- **Fixes checked before code.** You can check a fix in the spec, then replay every behavior of a small spec against
  the real TS code.

## Limitations

- **No proof about the TS code.** TLC checks the spec. The replay checks the code on the spec's behaviors, but not
  liveness.
- **The spec takes effort.** You choose where the steps split. If you merge steps that the real code keeps apart, the
  spec hides the bug.
- **Only what the spec models.** A connection that drops later, or more than one process, is not in this spec, so TLC
  says nothing about it.
- **Unfamiliar syntax.** `/\`, `'`, `EXCEPT` and `<>` take a day to learn. [Quint](/formal-method-tools/quint) runs the same
  checkers with a friendlier language.
...
- [Quint for TypeScript developers](/formal-method-tools/quint): a TLA+-based language with the same checkers underneath.
