...
- **effect-machine explores the machine that runs.** There is no second artifact. In exchange, it only checks code
...
- **The coverage report is a strength.** It named the exact transition that was never taken, and hard limits are
  reported as completeness, so a bounded run is never mistaken for an exhaustive one.

## Who builds it

Sandro Maglione of Typeonce, who writes and teaches about Effect, makes nearly all the commits.
