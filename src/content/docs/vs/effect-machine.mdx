...
[Effect](https://effect.website), published on npm as `@typeonce/effect-machine`. Its testing module
includes a bounded breadth-first explorer with invariants and shortest counterexamples.
...
<Short>States and events come from Effect schemas; `Machine.make(...).handle(...)` says what each event does.</Short>
...
`MachineTest.explore(machine, options)` walks the machine breadth-first. The result has `stats`, a `completeness` tag, and transition coverage. A failure comes back as an error with the
...
not run invokes. Coverage says so directly: the one missed transition is the invoke's `onDone`. At runtime, according to its
...
(Tagging each reply with a cart version and rejecting an old one would also work.) The same exploration, with every
