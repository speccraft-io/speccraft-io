...
- **It does not control the order of async calls in TypeScript yet.** Every case ran one order of events, so other
  orders of the reservations and their saves were never tried. [fast-check](/vs/fast-check) can try random orders of
  promises, and [pnueli](/vs/pnueli), [TLA+](/tools/tla-plus) or [Quint](/tools/quint) check every order in a model.
