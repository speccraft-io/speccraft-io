...
<Short>A lease can expire while a node is paused, and its late write lands over newer data.</Short>
...
`lock.ts` and on the fixed copy, `lock.fixed.ts`:
...
- **`lock`** is the module with the transitions, `lock.ts` or `lock.fixed.ts`. `store` is only a label for the
  output.
...
<Short>The storage rejects older tokens and safety holds. The liveness check shows n0 can still be fenced out forever.</Short>

The fix is one branch in the write rule: the storage rejects a write that carries an older token than the newest one
it took. `lock.fixed.ts` is a copy of `lock.ts` with only that branch added.

<CodeFix before={before} after={after} title="lock.ts" />

With `lock.fixed.ts` the invariant holds. States visited to prove it, with no reduction, with symmetry, and with symmetry
...
| Output | Verdict per invariant, trace, liveness cycle |
...
- **All in-process TypeScript.** No Java and no translation step, unlike [stateproof](/vs/stateproof) and
  ...
  [TLA+](/tools/tla-plus) with TLC covers more.

## Who builds it

Doniyor Botirov, founder of dbit.one, alongside his other testing tools unflake, bulwark and adya.
