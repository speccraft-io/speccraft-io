...
- **Declare `reads` and `writes` honestly.** An undeclared field makes partial-order reduction unsound, and a wrong
  ...
  `checkReduced` on every size and expects the same result from both.
...
| | pnueli |
|---|---|
| An action's result | A list of next states (several outcomes allowed) |
| Reductions | Symmetry and partial-order, validated against the full search |
| Liveness | "Eventually P" under weak fairness |
| Output | Result per invariant, trace, liveness cycle |
| Real code | Not covered: "the specification is not the implementation" |
| Async in real code | Not covered |

- **Liveness finds what a safety check cannot.** The fenced spec passes every invariant, and only the liveness check
  shows that n0 can be shut out forever.
- **Symmetry saves a lot of states.** It cut the proof from 2,521 states to 119 at four nodes.
- **It checks the model, not the code.** pnueli proves the model and stops there; its author points to separate
  sampling tools (unflake, bulwark) for the code.
- **All in-process TypeScript.** No Java and no translation step, unlike [stateproof](/vs/stateproof) and
  [tla-precheck](/vs/tla-precheck), which hand the model to TLC. For full temporal logic or strong fairness,
  [TLA+](/tools/tla-plus) with TLC covers more. States are kept in memory only; there is no disk-backed storage.
- **Tested on problems with known answers.** Peterson's algorithm, dining philosophers, Raft election safety (up to
  6.8M states) and write skew under snapshot isolation.
