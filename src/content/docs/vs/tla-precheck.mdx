...
adoption:
  ...
  requires:
    - 'Java 17 or newer'
    - 'TLC (tla2tools.jar), downloaded by setup'
    - 'Bun and Postgres, only for verify-db'
    - 'Agent skill for Claude Code or Codex (optional)'
---
...
best answer so far to the t## Using tla-precheck
...
ard, update and invariant must be built from the 13 expression kinds.
- **Java is required** for `check` and `build`; `verify-db`, which compares a live Postgres schema with the generated
  constraints, also needs Bun.
