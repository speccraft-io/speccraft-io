...
description: tla-precheck compiles a restricted TypeScript DSL to both TLA+ and a TypeScript interpreter, proves the two state graphs identical, and generates the runtime code. This page walks through using it on a ticket shop with a per-customer limit bug, then compares it with SpecCraft, which runs a spec in full TypeScript and checks separate code against it.
