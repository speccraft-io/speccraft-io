...
The same spec as a plain TypeScript object, in the shape that [SpecCraft TS](/typescript-formal-methods/speccraft-ts) takes. Other
TypeScript model checkers use a similar shape; in [pnueli](/typescript-formal-methods/pnueli), a step returns a list of next states instead of
one.
...
The same spec in [Quint](/formal-methods/quint). Each action lists the next value of every variable (`status'`), and anything
...
- [State machines and FSMs](/concepts/state-machines): why a model checker's spec lists variables and steps, not
  states.
- [TLA+ for TypeScript developers](/formal-methods/tla-plus) and [Quint for TypeScript developers](/formal-methods/quint): the spec
