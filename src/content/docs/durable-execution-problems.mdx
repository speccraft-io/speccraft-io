...
But none of this is free. The promise is real, and it is expensive: workers to run and tune, the determinism constraints to respect, and a whole web of signals, timers, and handlers to get right — operating durable workflows is anything but trivial, and it can be genuinely painful. And increasingly this is the substrate for AI agentic systems, whose orchestrating agent is non-deterministic by nature. Temporal boxes that non-determinism inside activities — but the agent's unpredictable choices still steer which handlers and branches fire, so the interleavings these bugs hide in are driven by something you can't predict up front. Unbounded AI unpredictability meeting a strict, concurrency-heavy execution model is an unforgiving combination.

Once you have paid that cost, the bugs that remain live in your workflow's own logic. You always own the fix — the platform never applies one for you — but the bugs split into **two families** by where the answer comes from: the ones the platform already has a tool or pattern for, and the ones no generic tool can even find.
