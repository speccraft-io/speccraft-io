...
- **It does not control the order of async calls in TypeScript yet.** Every case ran one order of events, so other
  orders of the reservations and their saves were never tried. [fast-check](/typescript-formal-methods/fast-check) can try random orders of
  promises, and [pnueli](/typescript-formal-methods/pnueli), [TLA+](/formal-methods/tla-plus) or [Quint](/formal-methods/quint) check every order in a model.
