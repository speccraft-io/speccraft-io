// The spec of cart.fixed.ts, as Polygraph's LLM step would derive it from the code: a SAM v2 strict-profile module.
'use strict';
  ...
  component: {
    ...
    acceptors: {
      ...
      REMOVE_ITEM: (model) => (proposal, { reject, next, unchanged }) => {
        ...
        const items = model.items - 1;
        next.items = items;
        next.coupon = items * 1000 < 2000 ? null : model.coupon;
      },
